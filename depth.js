...
(function () {
  ...
  var FRAG = [
    ...
    '  } else if(m==30){',                           /* Parallax */
    '    vec2 sw = vec2(sin(t*0.85), cos(t*0.62)*0.55)*0.055;',
    '    vec2 su = uv + sw*nd;',
    '    su = (su-0.5)/1.06 + 0.5;',
    '    c = albAt(clamp(su,0.002,0.998))*1.05;',
    '    float vg = 1.0-0.5*dot(uv-0.5,uv-0.5)*2.2;',
    '    c *= vg;',
    '  } else if(m==31){',                           /* Anaglyph */
    '    float disp = (d-0.5)*0.035;',
    '    vec3 L = albAt(clamp(uv-vec2(disp,0.0),0.002,0.998));',
    '    vec3 R = albAt(clamp(uv+vec2(disp,0.0),0.002,0.998));',
    '    c = vec3(dot(L,vec3(0.45,0.50,0.18)), R.g*0.95, R.b*1.05);',
    '  } else if(m==32){',                           /* Aperture */
    ...
    '      vec3 s = albAt(clamp(uv+o,0.002,0.998));',
    '      float w = 1.0 + pow(max(max(s.r,s.g),s.b),4.0)*3.0;',
    '      acc += s*w; wsum += w;',
    '    }',
    '    c = acc/max(wsum,0.001);',
    '    c += vec3(0.9,1.0,0.3)*smoothstep(0.14,0.02,coc)*0.05;',
    '  } else if(m==33){',                           /* Nebula */
    ...
    '    vec3 lit = mix(vec3(0.10,0.13,0.22), vec3(1.0,0.92,0.82), sh);',
    '    c = mix(albAt(uv)*0.30, lit, clamp(cloud*1.5,0.0,1.0));',
    '  } else if(m==34){',                           /* Motes */
    '    c = albAt(uv)*0.32;',
    '    for(int L=0;L<3;L++){',
    '      float fl = float(L);',
    ...
    '      c += mix(vec3(1.0,0.95,0.85), vec3(0.85,0.9,1.0), hash21(cid+fl+2.0))*m0*tw*0.55;',
    '    }',
    '  } else if(m==35){',                           /* Woodblock */
    '    vec3 a2 = albAt(uv);',
    '    float tone = 1.0-clamp(luma(a2)*0.55 + (1.0-d)*0.55, 0.0, 1.0);',
    ...
    '    float sil = smoothstep(0.12,0.40,length(vec2(',
    '        depthAt(uv+vec2(0.004,0.0))-depthAt(uv-vec2(0.004,0.0)),',
    '        depthAt(uv+vec2(0.0,0.004))-depthAt(uv-vec2(0.0,0.004))))*20.0);',
    '    c = mix(vec3(0.94,0.91,0.84), vec3(0.08,0.06,0.05), max(ink, sil));',
    '    c *= 0.92+0.16*fbm(uv*vec2(160.0,200.0));',
    '  } else if(m==36){',                           /* Stipple */
    '    vec3 a2 = albAt(uv);',
    '    float tone = 1.0-clamp(luma(a2)*0.95 + clamp(n.z,0.0,1.0)*0.22 - 0.18, 0.0, 1.0);',
    '    tone = pow(clamp(tone*1.35,0.0,1.0), 1.5);',
    ...
    '    c = albAt(uv)*0.22 + wire*(line*0.9 + node*1.3)*(0.35+1.1*band);',
    '  } else if(m==38){',                           /* Datamosh */
    ...
    '    vec3 s = albAt(clamp(su,0.002,0.998));',
    '    s = floor(s*7.0)/7.0;',
    '    vec3 s2 = albAt(clamp(su+mv*0.03,0.002,0.998));',
    '    c = vec3(s.r, mix(s.g,s2.g,0.55), s2.b);',
    '  } else if(m==39){',                           /* Aerial */
    '    vec3 a2 = albAt(uv);',
    ...
    '    c = mix(vec3(0.55,0.60,0.68), c, 1.0-0.30*ex);',
    '  } else if(m==40){',                           /* Godlight */
    ...
    '      su -= dir;',
    '      vec3 s = albAt(clamp(su,0.002,0.998));',
    '      float bright = smoothstep(0.86,1.0,luma(s))*step(0.62,depthAt(su));',
    '      sum += bright*dec; dec *= 0.93;',
    '    }',
    '    c = a2*0.22 + vec3(1.0,0.93,0.78)*sum*0.32;',
    '  } else if(m==41){',                           /* Hologram */
    '    vec3 a2 = albAt(uv);',
    ...
    '    c = tint*(0.20+0.85*luma(a2))*scan + tint*rim*1.2;',
    '    c += vec3(0.4,0.9,1.0)*gl*0.35;',
    '    c *= 0.35+1.0*smoothstep(0.95,0.30,d);',
    '  } else if(m==42){',                           /* Papercut */
    ...
    '  } else if(m==43){',                           /* Riso */
    '    vec3 a2 = albAt(uv);',
    '    float tone = clamp(luma(a2)*0.55 + (1.0-d)*0.62, 0.0, 1.0);',
